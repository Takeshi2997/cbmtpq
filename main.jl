include("./setup.jl")
include("./ml_core.jl")
include("./functions.jl")
using .Const, .MLcore, .Func, LinearAlgebra, Serialization, InteractiveUtils

function main()

    dirname = "./data"

    f = open("error.txt", "w")
    for iϵ in 1:1 #Const.iϵmax
    
        ϵ = (0.0 - 0.5 * (iϵ - 1) / Const.iϵmax) * Const.t * Const.dimB

        filename = dirname * "/param_at_" * lpad(iϵ, 3, "0") * ".dat"
        filenameinit = dirname * "/param_at_000.dat"

        # Initialize weight, bias
        wmoment  = zeros(Complex{Float32}, Const.dimH, Const.dimV)
        bmomentH = zeros(Complex{Float32}, Const.dimH)
        bmomentV = zeros(Complex{Float32}, Const.dimV)
        error   = 0.0
        energyS = 0.0
        energyB = 0.0
        lr = Const.lr

        # Define network
        params  = open(deserialize, filenameinit)
        network = MLcore.Network(params...)

        # Learning
        for it in 1:Const.it_num

            error, energyS, energyB, numberB, 
            dweight, dbiasH, dbiasV = 
            MLcore.diff_error(network, ϵ)

            # Optimize
            wmoment         = 0.9 * wmoment - lr * dweight
            network.weight += wmoment
            bmomentH        = 0.9 * bmomentH - lr * dbiasH
            network.biasH  += bmomentH
            bmomentV        = 0.9 * bmomentV - lr * dbiasV
            network.biasV  += bmomentV

            write(f, string(it))
            write(f, "\t")
            write(f, string(error))
            write(f, "\t")
            write(f, string(energyS / Const.dimS))
            write(f, "\t")
            write(f, string(energyB / Const.dimB))
            write(f, "\t")
            write(f, string(numberB / Const.dimB))
            write(f, "\n")
        end
   
        # Write error
#        write(f, string(iϵ))
#        write(f, "\t")
#        write(f, string(error))
#        write(f, "\t")
#        write(f, string(energyB / Const.dimB))
#        write(f, "\t")
#        write(f, string(energyS / Const.dimS))
#        write(f, "\t")
#        write(f, string(numberB / Const.dimB))
#        write(f, "\n")

        params = (network.weight, network.biasH, network.biasV)
        open(io -> serialize(io, params), filename, "w")
    end
    close(f)
end

@time main()

