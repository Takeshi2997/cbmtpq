...
function updateS(x::Array{Float32, 1})

    for ix in 1:Const.dimS
        z = ANN.forward(x)
        xflip = flip(x, ix)
        zflip = ANN.forward(xflip)
        rate = exp(2.0f0 * real(zflip .- z))
        if 1.0f0 > rate
            prob = rand(Float32)
            if prob < rate
                x[ix] *= -1.0f0
...
function flip(x::Array{Float32, 1}, ix::Integer)

    xout = copy(x)
    if ix <= Const.dimS
        xout[ix] *= -1.0f0
    else
        xout[ix] = 1.0f0 - xout[ix]
    end

    return xout
end

function flip(x::Array{Float32, 1}, iy::Integer, ix::Integer)
    ...
    if ix <= Const.dimS
        ...
        xout[ix] = 1.0f0 - xout[ix]
        xout[iy] = 1.0f0 - xout[iy]
    end

    return xout
end

function updateB(x::Array{Float32, 1})
...
function hamiltonianS_shift(x::Array{Float32, 1}, 
...
function energyS_shift(x::Array{Float32, 1})
...
function hamiltonianB_shift(x::Array{Float32, 1}, 
...
function energyB_shift(x::Array{Float32, 1})
